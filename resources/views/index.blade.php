...
function getAqiCategory(aqiValue) {
    if (aqiValue >= 0 && aqiValue <= 50) {
        ...
        return { category: 'Hazardous', color: 'darkred' };
    }
}
...
marker.addListener('click', () => {
    ...
    document.getElementById('sensor-name').textContent = location.name; // Update the sensor name here
    ...
    fetch(`/sensor/${location.id}/historical-aqi`)
        ...
        .then(data => {
            ...
            JSC.chart(`aqi-chart-${location.id}`, {
                ...
                series: [{
                    name: 'AQI',
                    points: aqiValues.map((value, index) => [times[index], value])
                }]
            });
        })
